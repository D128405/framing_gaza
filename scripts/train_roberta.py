...
OUTPUT_DIR = "results/roberta/trained_models/roberta_labelframes"
MAX_LEN = 512
...
class MultiLabelDataset(Dataset):
    """Dataset wrapper for multi-label text classification."""
    ...
    def __getitem__(self, idx):
        """Tokenize one text instance and return model-ready tensors."""
        text = str(self.texts[idx])
        labels = self.labels[idx]

        # Tokenize the input text and return fixed-length tensors.
...
def load_data(filepath):
    """Load text data and convert frame annotations to binary label vectors."""
...
if __name__ == "__main__":
    ...
    with open(os.path.join(OUTPUT_DIR, "label2id_roberta_labelframes.json"), "w") as f:
    ...
    training_args = TrainingArguments(
        ...
        gradient_accumulation_steps=4,        # Number of steps over which gradients are accumulated.
        per_device_eval_batch_size=2,         # Evaluation batch size per device.
        num_train_epochs=3,
        weight_decay=0.01,
        load_best_model_at_end=True,
        metric_for_best_model="eval_loss",
        logging_steps=1,                      # Log training metrics after every optimization step.
    ...
    print("RoBERTa-base training complete and saved.")