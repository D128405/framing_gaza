...
load_cluster_data <- function(pattern = "\\.csv$") {
  ...
  map_dfr(files, function(file) {
    ...
    d <- d %>% mutate(Country = vapply(`News Outlet`, classify_country, character(1)))
...
if (!is.null(df_rq1)) {
  ...
  rq1_desc <- df_rq1 %>%
    ...
    summarize(N = n(), Present = sum(Present), Proportion = Present / N, .groups = "drop") %>%
...
if (!is.null(df_rq2)) {
  ...
  fit_rows <- list(); sparsity_rows <- list(); firth_rows <- list()
  vif_rows <- list()
  me_plots_rq2 <- list()
  ...
  for (fr in FRAMES) {
    ...
    fit_rows[[fr]] <- model_fit_row(m_full, fr, "Interactive")
    
    m_add <- safe_glm(Present ~ Alignment + Period, d)
    coef_add_rows[[fr]] <- tidy_or(m_add, fr)
    fit_rows[[paste0(fr, "_add")]] <- model_fit_row(m_add, fr, "Additive")
  ...
  rq2_fit       <- bind_rows(fit_rows)
  rq2_sparsity  <- bind_rows(sparsity_rows)
  rq2_vif       <- bind_rows(vif_rows)
  ...
  write_csv(rq2_fit,      file.path(OUTPUT_DIR, "rq2_model_fit.csv"))
  write_csv(rq2_sparsity, file.path(OUTPUT_DIR, "rq2_sparsity_check.csv"))
  if (nrow(rq2_vif) > 0) write_csv(rq2_vif, file.path(OUTPUT_DIR, "rq2_vif_diagnostics.csv"))
  if (length(firth_rows) > 0) {
    write_csv(bind_rows(firth_rows), file.path(OUTPUT_DIR, "rq2_firth_sensitivity.csv"))
  }
...
if (!is.null(df_rq3)) {
  ...
  coef_ord_rows <- list(); coef_ord_add_rows <- list()
  ...
  for (fr in FRAMES) {
    ...
    lin_test_rows[[fr]] <- tibble(
      ...
      Ordinal_AIC_Weight = c(w_add[1], w_int[1]),
      Nominal_AIC_Weight = c(w_add[2], w_int[2])
    )
  ...
  if (nrow(rq3_vif) > 0) write_csv(rq3_vif, file.path(OUTPUT_DIR, "rq3_vif_diagnostics.csv"))
  if (length(firth_rows) > 0) {
